<template>
  <div>
    <el-menu background-color="#453549" text-color="#fff" active-text-color="#45a0fc" class="hidden-xs-only el-menu-demo" :default-active="activeIndex" mode="horizontal" @select="handleSelect">
      ...
      <el-submenu index="2">
        <template slot="title">InDaDogs</template>
        ...
        <el-menu-item class="user-container">
          <span class="user" v-if="isAuthenticated">
              Hi<span class="nome-utente"> {{getUsername}} </span>
              <img class="user-icon" src="./../assets/icons/user.svg"> |  <img class="user-icon logout" src="./../assets/icons/logout.svg">
          </span>
          <span v-else class="user"> <router-link to="login">  LOGIN   <img class="user-icon login" src="./../assets/icons/login.svg"></router-link></span>
        </el-menu-item >
      </el-menu>
        <div class="line"></div>
